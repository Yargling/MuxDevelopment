// game.cpp
//
// $Id: game.cpp 3050 2007-12-30 06:36:06Z brazilofmux $
//
#include "copyright.h"
#include "autoconf.h"
#include "config.h"
...
void report(void) {
	...
	if (Good_obj(mudstate.curr_executor)) {
		...
		if (mudstate.curr_enactor
				!= mudstate.curr_executor&& Good_obj(mudstate.curr_enactor)) {
			log_text(" Enactor: ");
...
bool regexp_match(char *pattern, char *str, int case_opt, char *args[],
		int nargs) {
	int matches;
	int i;
	const char *errptr;
	...
	if (MuxAlarm.bAlarmed
			|| (re = pcre_compile(pattern, case_opt, &errptr, &erroffset, NULL))
					== NULL) {
		/*
	 ...
	 */
	matches = pcre_exec(re, NULL, str, static_cast<int>(strlen(str)), 0, 0,
			ovec, ovecsize);
	if (matches < 0) {
		delete[] ovec;
		MEMFREE(re);
	...
	for (i = 0; i < nargs; ++i) {
		...
		if (pcre_copy_substring(str, ovec, matches, i, args[i], LBUF_SIZE)
				< 0) {
			free_lbuf(args[i]);
			args[i] = NULL;
		}
...
static int atr_match1(dbref thing, dbref parent, dbref player, char type,
		char *str, char *raw_str, int check_exclude, int hash_insert) {
	...
	int match = 0;
	if ((AMATCH_CMD == type && mudstate.bfNoCommands.IsSet(parent))
			|| (AMATCH_LISTEN == type && mudstate.bfNoListens.IsSet(parent))) {
		...
		if (hash_insert) {
			...
			for (int atr = atr_head(parent, &as); atr; atr = atr_next(&as)) {
				...
				if (ap && 0 == (ap->flags & AF_NOPROG)
						&& (!check_exclude
								|| (0 == (ap->flags & AF_PRIVATE)
										&& !hashfindLEN(&(ap->number),
												sizeof(ap->number),
												&mudstate.parent_htab)))) {
					hashaddLEN(&(ap->number), sizeof(ap->number), &atr,
							&mudstate.parent_htab);
				}
			}
			atr_pop();
	...
	for (int atr = atr_head(parent, &as); atr; atr = atr_next(&as)) {
		...
		if (0 == (aflags & AF_NOPROG)
				&& (AMATCH_CMD == buff[0] || AMATCH_LISTEN == buff[0])) {
			s = strchr(buff + 1, ':');
			if (s) {
		...
		//
		if (check_exclude
				&& ((ap->flags & AF_PRIVATE) || (aflags & AF_PRIVATE)
						|| hashfindLEN(&(ap->number), sizeof(ap->number),
								&mudstate.parent_htab))) {
			continue;
		}
		...
		*s++ = '\0';
		...
		if ((0 != (aflags & AF_REGEXP)
				&& regexp_match(buff + 1, (aflags & AF_NOPARSE) ? raw_str : str,
						((aflags & AF_CASE) ? 0 : PCRE_CASELESS), args,
						NUM_ENV_VARS))
				|| (0 == (aflags & AF_REGEXP)
			...
			wait_que(thing, player, player, AttrTrace(aflags, 0), false, lta,
					NOTHING, 0, s, NUM_ENV_VARS, args, mudstate.global_regs);
...
bool atr_match(dbref thing, dbref player, char type, char *str, char *raw_str,
		bool check_parents) {
	int lev, result;
	bool exclude, insert;
	dbref parent;
	...
	//
	if (Halted(thing) || (AMATCH_CMD == type && No_Command(thing))) {
		return false;
	}
	...
	bool match = false;
	if (!check_parents) {
		return (atr_match1(thing, thing, player, type, str, raw_str, false,
				false) > 0);
	}
	...
	ITER_PARENTS(thing, parent, lev)
	{
		...
		result = atr_match1(thing, parent, player, type, str, raw_str, exclude,
				insert);
		if (result > 0) {
			match = true;
		} else if (result < 0) {
...
static bool check_filter(dbref object, dbref player, int filter,
		const char *msg_arg) {
	int aflags;
	dbref aowner;
	...
	*mp = '\0';
	...
	mux_exec(nbuf, &dp, object, player, player,
			AttrTrace(aflags, EV_FIGNORE|EV_EVAL|EV_TOP), &str, NULL, 0);
	*dp = '\0';
	dp = nbuf;
	free_lbuf(buf);
	...
	if (!(aflags & AF_REGEXP)) {
		do {
			...
			if (MuxAlarm.bAlarmed || quick_wild(cp, msg)) {
				free_lbuf(nbuf);
				free_lbuf(msg);
				return false;
			}
		...
		do {
			...
			if (!MuxAlarm.bAlarmed
				...
				int ovec[ovecsize];
				int matches = pcre_exec(re, NULL, msg,
						static_cast<int>(strlen(msg)), 0, 0, ovec, ovecsize);
				if (0 <= matches) {
					MEMFREE(re);
					free_lbuf(nbuf);
					free_lbuf(msg);
...
void notify_except(dbref loc, dbref player, dbref exception, const char *msg,
	...
	if (loc != exception) {
		notify_check(loc, player, msg,
				(MSG_ME_ALL | MSG_F_UP | MSG_S_INSIDE | MSG_NBR_EXITS_A | key));
	}
...
static void report_timecheck(dbref player, bool yes_screen, bool yes_log,
		bool yes_clear) {
	...
	if (yes_log && (LOG_TIMEUSE & mudconf.log_options)) {
		start_log("OBJ", "CPU");
		log_name(player);
...
void do_shutdown(dbref executor, dbref caller, dbref enactor, int eval, int key,
		char *message) {
	UNUSED_PARAMETER(caller);
	UNUSED_PARAMETER(enactor);
	...
	int fd;
	if (mux_open(&fd, mudconf.status_file,
...
static DUMP_PROCEDURE DumpProcedures[NUM_DUMP_TYPES] = {
		...
		{ &mudconf.crashdb, "", false, UNLOAD_VERSION | UNLOAD_FLAGS,
				"Opening crash file" }, // 1
		{ &mudconf.indb, "", true, OUTPUT_VERSION | OUTPUT_FLAGS,
				"Opening input file" }, // 2
		{ &mudconf.indb, ".FLAT", false, UNLOAD_VERSION | UNLOAD_FLAGS,
				"Opening flatfile" }, // 3
		{ &mudconf.indb, ".SIG", false, UNLOAD_VERSION | UNLOAD_FLAGS,
				"Opening signalled flatfile" }  // 4
};
...
void dump_database_internal(int dump_type) {
	...
	if (dump_type < 0 || NUM_DUMP_TYPES <= dump_type) {
		return;
	}
	...
	//
	if (mudstate.dumping
			&& (dump_type == DUMP_I_PANIC || dump_type == DUMP_I_SIGNAL)) {
	...
	if (0 < dump_type) {
		...
		mux_sprintf(outfn, sizeof(outfn), "%s%s", *(dp->ppszOutputBase),
				dp->szOutputSuffix);
		if (dp->bUseTemporary) {
			mux_sprintf(tmpfile, sizeof(tmpfile), "%s.#%d#", outfn,
					mudstate.epoch);
			RemoveFile(tmpfile);
	...
	if (mudconf.compress_db) {
		...
		mux_sprintf(tmpfile, sizeof(tmpfile), "%s.#%d#.gz", mudconf.outdb,
				mudstate.epoch - 1);
		RemoveFile(tmpfile);
		mux_sprintf(tmpfile, sizeof(tmpfile), "%s.#%d#.gz", mudconf.outdb,
				mudstate.epoch);
		...
		f = popen(tprintf("%s > %s", mudconf.compress, tmpfile),
				POPEN_WRITE_OP);
		if (f) {
			DebugTotalFiles++;
			setvbuf(f, NULL, _IOFBF, 16384);
			...
			if (ReplaceFile(tmpfile, outfn) < 0) {
				log_perror("SAV", "FAIL", "Renaming output file to DB file",
						tmpfile);
			}
		} else {
		...
		mux_sprintf(tmpfile, sizeof(tmpfile), "%s.#%d#", mudconf.outdb,
				mudstate.epoch - 1);
		RemoveFile(tmpfile);
		mux_sprintf(tmpfile, sizeof(tmpfile), "%s.#%d#", mudconf.outdb,
				mudstate.epoch);
...
void fork_and_dump(int key) {
	...
	//
	if (bRequestAccepted || mudstate.dumping) {
		return;
	}
	...
	if (key & (DUMP_TEXT | DUMP_STRUCT)) {
		...
		if (key & DUMP_STRUCT) {
			mudstate.epoch++;
			mux_sprintf(buff, LBUF_SIZE, "%s.#%d#", mudconf.outdb,
					mudstate.epoch);
...
static int load_game(int ccPageFile)
	...
	if (mudconf.compress_db) {
		...
		if (stat(infile, &statbuf) == 0) {
			f = popen(tprintf(" %s < %s", mudconf.uncompress, infile),
					POPEN_READ_OP);
			if (f != NULL) {
				DebugTotalFiles++;
				compressed = true;
	...
	if (db_flags & V_DATABASE) {
		...
		if (ccPageFile == HF_OPEN_STATUS_NEW) {
			STARTLOG(LOG_STARTUP, "INI", "LOAD");
			log_text(
					"Attributes are not present in either the input file or the attribute database.");
			ENDLOG
			;
		}
	} else {
		...
		if (ccPageFile == HF_OPEN_STATUS_OLD) {
			STARTLOG(LOG_STARTUP, "INI", "LOAD");
			log_text(
					"Attributes present in both the input file and the attribute database.");
			ENDLOG
			;
		}
	}
...
bool list_check(dbref thing, dbref player, char type, char *str, char *raw_str,
		bool check_parent) {
	bool bMatch = false;
...
#endif // REALITY_LVLS
		{
			bMatch |= atr_match(thing, player, type, str, raw_str,
					check_parent);
		}
	...
	if (Monitor(thing)) {
		if (mudstate.bfListens.IsSet(thing)) {
			...
			for (int atr = atr_head(thing, &as); atr; atr = atr_next(&as)) {
				...
				char *s = NULL;
				if (AMATCH_CMD == buff[0] || AMATCH_LISTEN == buff[0]) {
					s = strchr(buff + 1, ':');
					if (s) {
						if (AMATCH_CMD == buff[0]) {
							bFoundCommands = true;
...
static void info(int fmt, int flags, int ver) {
	...
	if (ver < MIN_SUPPORTED_VERSION || MAX_SUPPORTED_VERSION < ver) {
		Log.WriteString(" Unsupported version");
		exit(1);
	} else if (((1 == ver || 2 == ver) && (flags & MANDFLAGS_V2)!= MANDFLAGS_V2)
	|| ( 3 == ver
...
static void dbconvert(void) {
	...
	safe_copy_str(standalone_basename, dirfile, &dirfile_c,
			(SIZEOF_PATHNAME - 1));
	...
	*dirfile_c = '\0';
	...
	safe_copy_str(standalone_basename, pagfile, &pagfile_c,
			(SIZEOF_PATHNAME - 1));
...
static CLI_OptionEntry OptionTable[NUM_CLI_OPTIONS] = { { "c", CLI_REQUIRED,
CLI_DO_CONFIG_FILE }, { "s", CLI_NONE, CLI_DO_MINIMAL }, { "v", CLI_NONE,
		CLI_DO_VERSION }, { "h", CLI_NONE, CLI_DO_USAGE },
#ifndef MEMORY_BASED
		{ "i", CLI_REQUIRED, CLI_DO_INFILE }, { "o", CLI_REQUIRED,
		CLI_DO_OUTFILE }, { "k", CLI_NONE, CLI_DO_CHECK }, { "l", CLI_NONE,
				CLI_DO_LOAD }, { "u", CLI_NONE, CLI_DO_UNLOAD }, { "d",
				CLI_REQUIRED, CLI_DO_BASENAME },
#endif // MEMORY_BASED
#ifdef WIN32
		{	"n", CLI_NONE, CLI_DO_SELECT},
#endif // WIN32
		{ "p", CLI_REQUIRED, CLI_DO_PID_FILE }, { "e", CLI_REQUIRED,
		CLI_DO_ERRORPATH } };
...
int DCL_CDECL main(int argc, char *argv[]) {
	...
	while (nProg && (mux_isalpha(*pProg)|| *pProg == '.'))
	{
		nProg--;
		pProg--;
	}
	...
	if (mudstate.bStandAlone) {
		...
		if (!standalone_basename || !standalone_infile || !standalone_outfile
				|| n != 1 || bServerOption) {
			bSyntaxError = true;
		} else {
			dbconvert();
	...
	if (bSyntaxError || conffile == NULL || !bServerOption) {
		fprintf(stderr, "Version: %s" ENDLINE, mudstate.version);
		if (mudstate.bStandAlone) {
			fprintf(stderr,
					"Usage: %s -d <dbname> -i <infile> [-o <outfile>] [-l|-u|-k]" ENDLINE,
					pProg);
			...
			fprintf(stderr,
					"Usage: %s [-c <filename>] [-p <filename>] [-h] [-s] [-v]" ENDLINE,
					pProg);
			fprintf(stderr, "  -c  Specify configuration file." ENDLINE);
			fprintf(stderr,
					"  -e  Specify logfile basename (or '-' for stderr)." ENDLINE);
	...
	Log.WriteString("Setting up ports: "ENDLINE);
	Log.WriteInteger(nMainGamePorts);
	Log.WriteString(ENDLINE);
	Log.WriteString(toString(aMainGamePorts, 10).c_str());
	Log.WriteString(ENDLINE);
	Log.WriteString(toString(mudconf.ports).c_str());
	Log.WriteString(ENDLINE);
	SetupPorts(&nMainGamePorts, aMainGamePorts, &mudconf.ports);
	Log.WriteString("Finished port setup"ENDLINE);
	Log.WriteInteger(nMainGamePorts);
	Log.WriteString(ENDLINE);
	Log.WriteString(toString(aMainGamePorts, 10).c_str());
	Log.WriteString(ENDLINE);
	...
	aMainGamePorts[0].type = WEB_SOCKET;
...
#else // WIN32
	Log.WriteString("Got to 'shovechars'"ENDLINE);
	Log.Flush();
