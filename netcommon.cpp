// netcommon.cpp
//
// $Id: netcommon.cpp 2823 2007-11-28 03:33:36Z brazilofmux $
//
...
void make_portlist(dbref player, dbref target, char *buff, char **bufc)
{
    ...
    {
        if (  d->player == target
           && !ItemToList_AddInteger64(&itl, d->getSocket()))
        {
            break;
        }
    }
...
void queue_write_LEN(DESC *d, const char *b, size_t n)
{
    ...
    if (static_cast<size_t>(mudconf.output_limit) < d->output_size + n)
    {
        ...
        else
        {
            ...
            mux_sprintf(buf, LBUF_SIZE, "[%u/%s] Output buffer overflow, %d chars discarded by ", d->getSocket(), d->addr, tp->hdr.nchars);
            log_text(buf);
            free_lbuf(buf);
                     ...
                     char *user, char *password, const char *cmdsave)
{
    ...
    mux_sprintf(buff, MBUF_SIZE, "[%u/%s] %s rejected to ", d->getSocket(), d->addr, logtype);
    log_text(buff);
    free_mbuf(buff);
    if (player != NOTHING)
    {
...
static bool check_connect(DESC *d, char *msg)
{
       ...
       || strncmp(command, "cd", 2) == 0)
    {
           ...
           || (!isGuest && Guest.CheckGuest(player)))
        {
            ...
            mux_sprintf(buff, LBUF_SIZE, "[%u/%s] Failed connect to '%s'", d->getSocket(), d->addr, user);
            log_text(buff);
            free_lbuf(buff);
            ENDLOG;
            if (--(d->retries_left) <= 0)
                ...
                || God(player))
        {
            ...
            mux_sprintf(buff, MBUF_SIZE, "[%u/%s] Connected to ", d->getSocket(), d->addr);
            log_text(buff);
    ...
    else if (strncmp(command, "cr", 2) == 0)
    {
        ...
        else
        {
            ...
            if (player == NOTHING)
            {
                ...
                mux_sprintf(buff, LBUF_SIZE, "[%u/%s] Create of '%s' failed", d->getSocket(), d->addr, user);
                log_text(buff);
                free_lbuf(buff);
                ENDLOG;
            }
            else
            {
                ...
                mux_sprintf(buff, MBUF_SIZE, "[%u/%s] Created ", d->getSocket(), d->addr);
                log_text(buff);
                log_name(player);
                free_mbuf(buff);
                ENDLOG;
    ...
    else
    {
        ...
        msg[150] = '\0';
        mux_sprintf(buff, MBUF_SIZE, "[%u/%s] Failed connect: '%s'", d->getSocket(), d->addr, msg);
        log_text(buff);
        free_mbuf(buff);
        ENDLOG;
    }
